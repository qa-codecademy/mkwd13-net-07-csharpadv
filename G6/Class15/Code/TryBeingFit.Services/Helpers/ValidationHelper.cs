...
namespace TryBeingFit.Services.Helpers
{
    public static class ValidationHelper
    {
        ...
        public static bool ValidateTrainingDuration(double duration)
        {
            if (duration < 10)
            {
                return false; //if the duration is less than 10 min then the training duration is not valid
            }

            return true; //if the duration is not less than 10 min then the training duration is valid
        }
    }
}
